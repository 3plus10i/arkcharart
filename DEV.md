...
`内置` 字段：构建时通过检查 `文件名` 是否存在于 `public/chararts/` 目录来确定。
- `内置=true`：本地有此立绘资源，选中后可立即加载
- `内置=false`：本地无此资源，需从远端 `文件链接` 拉取
...
参数滑块（大小/位置/Logo/质量）变化时直接重组合成，无需确认。
...
2. `charScale` / `charPos` / `charYOffset` / `logoScale` / `selectedLogo` / `outputQuality` 变化 → `useEffect` 200ms debounce
...
```
1. 底图 (bg.svg)                     — 铺满 Canvas
2. 背景大立绘                         — 居右偏，高度=canvas×1.2×charScale
3. Logo                              — 居左上，高度=canvas×0.75×logoScale，带阴影
4. 白色渐变遮罩                       — 左半透明→右不透明，覆盖立绘
5. 中心人物立绘                       — 居中，高度=canvas×1.0×charScale
```
...
### 3.3 `src/lib/artListManager.js` — 立绘编号映射

提供 `codeToName(code)` 映射（1→精一, 2→精二, skinN→皮肤N）。

### 3.4 `src/config.js` — 合成参数常量

| 常量 | 默认值 | 含义 |
|------|--------|------|
| `CANVAS_WIDTH/HEIGHT` | 3840×2160 | 4K 画布尺寸 |
...
6. 自动设置：出处=用户上传, confirmedChar=角色名, confirmedSkinCode=1
